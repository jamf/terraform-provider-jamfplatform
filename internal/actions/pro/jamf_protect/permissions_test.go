...
// TestSyncPlansSDKMethods_KnownToSDK fails if a declared method has been renamed
// or removed in the SDK privilege registry.
func TestSyncPlansSDKMethods_KnownToSDK(t *testing.T) {
	if missing := permissions.Missing(pro.Privileges, syncPlansSDKMethods...); len(missing) > 0 {
		t.Fatalf("syncPlansSDKMethods not present in pro.Privileges (SDK drift): %v", missing)
	}
}

// TestSyncPlansSDKMethods_MatchCalls fails if sync_plans.go calls an SDK method
// not declared in syncPlansSDKMethods, or declares one it does not call.
func TestSyncPlansSDKMethods_MatchCalls(t *testing.T) {
	assertMatch(t, "sync_plans.go", syncPlansSDKMethods)
}
...
// TestRetryDeploymentSDKMethods_KnownToSDK fails if a declared method has been
// renamed or removed in the SDK privilege registry.
func TestRetryDeploymentSDKMethods_KnownToSDK(t *testing.T) {
	if missing := permissions.Missing(pro.Privileges, retryDeploymentSDKMethods...); len(missing) > 0 {
		t.Fatalf("retryDeploymentSDKMethods not present in pro.Privileges (SDK drift): %v", missing)
	}
}

// TestRetryDeploymentSDKMethods_MatchCalls fails if retry_deployment.go calls an
// SDK method not declared in retryDeploymentSDKMethods, or declares one it does
// not call. The computer resolver lives in the computertarget package, so its
// SDK calls are intentionally not scanned here (matching the redeploy action).
func TestRetryDeploymentSDKMethods_MatchCalls(t *testing.T) {
	assertMatch(t, "retry_deployment.go", retryDeploymentSDKMethods)
}
