...
func TestEnumLiteralsComeFromTheSDK(t *testing.T) {
	got, err := enumguard.Check(enumguard.Params{
		Covered: enumguard.Union(
			pro.PlanConfigurationPostUpdateActionValues(),
			pro.PlanConfigurationPostVersionTypeValues(),
			pro.PlanGroupPostObjectTypeValues(),
			pro.ManagedSoftwareUpdatePlanUpdateActionValues(),
			pro.ManagedSoftwareUpdatePlanVersionTypeValues(),
		),
	})
	if err != nil {
