...
// <receiver>.client.<Method>( — covering the r.client and d.client receivers
// used by the resource, the data sources and the list resource.
...
// so unrelated identifiers are ignored.
...
// the resource description, for all four actions the lifecycle needs.
func TestResourcePrivileges_Rendered(t *testing.T) {
	for _, scoped := range []string{
		"sso-connections:create",
		"sso-connections:read",
...
// upstream defect this namespace shipped with. The published specification
// declares no privileges for any Jamf Account operation, and an empty
// declaration renders as "None — any authenticated … integration may call the
// underlying endpoints", which for these operations is false. The SDK now
// supplies the gateway's own policy; this asserts the provider never publishes
// the false claim if that regresses.
...
func TestDataSourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(dataSourcePrivileges, "sso-connections:read") {
		t.Fatalf("dataSourcePrivileges did not render the SSO connection privileges:\n%s", dataSourcePrivileges)
	}
}

// TestDataSourcePrivileges_AskForNoWrite pins that reading a connection never
// asks for a privilege that could change one. A data source that asked for the
// write actions would make an operator provision more than they need.
func TestDataSourcePrivileges_AskForNoWrite(t *testing.T) {
	for _, section := range map[string]string{
		...
		for _, forbidden := range []string{
			...
			if permissions.Renders(section, forbidden) {
				t.Errorf("a read-only construct asks for %s:\n%s", forbidden, section)
			}
		}
	}
}
...
func TestPluralDataSourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(pluralDataSourcePrivileges, "sso-connections:read") {
		t.Fatalf("pluralDataSourcePrivileges did not render the SSO connection privileges:\n%s", pluralDataSourcePrivileges)
	}
}
...
func TestListResourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(listResourcePrivileges, "sso-connections:read") {
		t.Fatalf("listResourcePrivileges did not render the SSO connection privileges:\n%s", listResourcePrivileges)
	}
}
