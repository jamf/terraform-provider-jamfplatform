...
func TestDataSourceSDKMethods_KnownToSDK(t *testing.T) {
	if missing := permissions.Missing(devSDK.Privileges, dataSourceSDKMethods...); len(missing) > 0 {
		t.Fatalf("dataSourceSDKMethods not present in devices.Privileges (SDK drift): %v", missing)
	}
}
...
func TestDataSourceSDKMethods_MatchReadCalls(t *testing.T) {
	...
	// Restrict matches to method names known to the SDK registry so unrelated
	// client.<Method> helpers (none expected here) cannot fail the guard.
