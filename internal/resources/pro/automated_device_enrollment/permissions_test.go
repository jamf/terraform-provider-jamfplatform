...
// client.<Method> (waitForAdeSync takes the client as a parameter), the data
...
// (e.g. ResolveDeviceEnrollmentV1ByName) that the SDK privilege registry does
// not carry, so the comparison reflects exactly the privilege-bearing calls a
// construct makes.
...
func TestResourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(resourcePrivileges, "device-enrollment-program-instances:create") {
		t.Fatalf("resourcePrivileges did not render the device-enrollment privileges:\n%s", resourcePrivileges)
	}
}
...
func TestDataSourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(dataSourcePrivileges, "device-enrollment-program-instances:read") {
		t.Fatalf("dataSourcePrivileges did not render the device-enrollment privileges:\n%s", dataSourcePrivileges)
	}
}
...
func TestListResourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(listResourcePrivileges, "device-enrollment-program-instances:read") {
		t.Fatalf("listResourcePrivileges did not render the device-enrollment privileges:\n%s", listResourcePrivileges)
	}
}
