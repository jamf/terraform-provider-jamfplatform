...
func (r *StaticComputerGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		if recoveryErr := recordCreatedGroup(createCtx, r.client, &plan, created.ID, resp); recoveryErr != nil {
			resp.Diagnostics.AddError(
				"Created the Jamf Pro "+groupLabel+" but recorded nothing for it",
				"The group exists in Jamf Pro. Reading it back failed, Terraform then looked up the identifier it stores from a second endpoint, and that failed too, so state holds no record of the group. Its identifier across Jamf Platform is "+created.ID+
...
func recordCreatedGroup(ctx context.Context, client *pro.Client, plan *StaticComputerGroupResourceModel, platformID string, resp *resource.CreateResponse) error {
