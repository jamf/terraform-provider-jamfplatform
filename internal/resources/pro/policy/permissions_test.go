...
// "r.client.GetPolicyByID(" or "d.client.GetPolicyByName(".
var clientCallRE = regexp.MustCompile(`\bclient\.([A-Za-z0-9]+)\(`)

// sdkCallsIn returns the distinct SDK client method calls in the named source
// file, restricted to names present in the proclassic privilege registry so
// unrelated client helpers do not leak into the comparison.
...
// assertMethodsMatch fails if the source file's SDK calls differ from the
// declared method list — keeping the privileges table honest as the construct
// changes.
func assertMethodsMatch(t *testing.T, filename string, declared []string) {
	t.Helper()
...
func TestResourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(resourcePrivileges, "policies:create") {
...
func TestDataSourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(dataSourcePrivileges, "policies:read") {
...
func TestListResourcePrivileges_Rendered(t *testing.T) {
	if !permissions.Renders(listResourcePrivileges, "policies:read") {
