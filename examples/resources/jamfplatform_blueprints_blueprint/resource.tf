...
# `payloadIdentifier` is derived per Blueprint resource, step position, and payload type when the
# typed attribute builds the component, so state one explicitly — any stable UUID will do, as long
# as it does not change between applies.
